...
procedure LinkList(); //Лист полезных ссылок.
var
...
procedure AddPresent();//Процедура добавления призов в админке.
var
t_a_ptitle : String;
...
procedure PresentList(w_id : integer);   // Загруска листа призов.
var
sql_prlist : String;
i1, i2 : integer;
begin
...
procedure SetRating(plus : boolean);  //Управление рейтингом.
var
...
begin
  ...
  Form6.ZQueryNew1.SQL.Add(sql_rating);
  Form6.ZQueryNew1.ExecSQL;
  ShowMessage('Рейтинг изменен!');
  Form6.N17.Click;
end;

procedure InsertUser();//Добавить пользователя.
var
...
procedure ChatSend();//Процедура отправки сообщений в чате.
var
sql_send : String;
...
procedure MsgSend();//Процедура отправки сообщений в админке.
var
...
procedure SetPlatformsNotVisible();//Процедура установки невидимости всех панелей.
begin
...
begin
  ...
  8:begin
    ...
    ShowMessage('Задание отправлено на доработку!');
    end;
  9:begin
    ...
    ShowMessage('Задание отредактировано!');
    end;
  end;
...
procedure Tasks_ctrl(sost : string); //Загрузка задания в админ-бар.
var
i1 : integer;
sql1: String;
(*title, reward : String;
...
begin
  ...
  if Trunc(Form6.ZQuery_task.RecordCount.ToDouble)<>0 then
    ...
    begin
      ShowMessage('Список заданий пока пуст!');
      Form6.N17.Click;
    end;
...
procedure Tasks(sost : string); //Загрузка задания.
var
i1 : integer;
sql1 : String;
(*title, reward : String;
...
procedure Massages(readed : integer); //Загрузка сообщений.
var
i1 : integer;
sql1: String;
begin
  ...
  if ((Trunc(Form6.ZQuery_massages.RecordCount.ToDouble)<>0)) then
    ...
    if Form6.Panel_N5.Visible then
      begin
        ShowMessage('Список сообщений пока пуст!');
        Form6.N17.Click;
      end;
    end;
  if ((readed=0) and (Form6.DockTabSet_massages.Tabs.Count>0) and (Form6.Panel_N5.Visible=False)) then
    begin
      Form6.N5.Caption:='++Сообщения++';
      if Form6.CheckBox1.Checked then Form6.MediaPlayer1.Play;
      Form6.ZQuery_massages.Last;
      Form6.TrayIcon1.BalloonHint:='Тема:'+Form6.ZQuery_massages.FieldByName('title').AsString+#10#13+'"'+Form6.ZQuery_massages.FieldByName('massage').AsString+'"'+#10#13+'Кликните, чтобы посмотреть';
      Form6.TrayIcon1.BalloonTitle:='Новое сообщение';
...
procedure TForm6.N17Click(Sender: TObject); //Действия на панеле "Другая информация".
begin
  SetPlatformsNotVisible();
...
procedure TForm6.N18Click(Sender: TObject); //Действия на панеле "Список сотрудников".
var
sql_N18 : string;
i1, i2 : integer;
begin
...
procedure TForm6.N20Click(Sender: TObject);//Действия на панеле "Переписка".
var
sql_N18 : string;
i1, i2 : integer;
begin
...
procedure TForm6.N8Click(Sender: TObject);//Действия на панеле "Баланс - Рейтинг".
begin
  SetPlatformsNotVisible();
...
procedure TForm6.N9Click(Sender: TObject); //Действия на панеле "Полный рейтинг".
begin
  SetPlatformsNotVisible();
...
procedure TForm6.Timer_ChatTimer(Sender: TObject);   //Обновление чата
var
  ...
  begin
    ...
    if Trunc(ListBox_N20.ItemIndex.ToDouble)>=1 then
      ...
      begin
        addressee_id:=Trunc(ListBox_N20.ItemIndex.ToDouble);
        addressee_name:='Все';
        addressee_surname:='пользователи';
      ...
      begin
        ...
        if addressee_id=0 then
          begin
            ...
            richedit1.Lines.Add('['+msg_time+']['+ZQuery_dop.FieldByName('name').AsString+' '+ZQuery_dop.FieldByName('surname').AsString+' отправил Всем]: '+msg_text);
            ZQuery_dop.Active:=False;
          end
        else
          begin
            if worker_id=ZQuery_Chat.FieldByName('sender_id').AsInteger then richedit1.Lines.Add('['+msg_time+']['+name_user+' '+surname+' отправил '+addressee_name+' '+addressee_surname+']: '+msg_text)
            else richedit1.Lines.Add('['+msg_time+']['+addressee_name+' '+addressee_surname+' отправил '+name_user+' '+surname+']: '+msg_text);
          end;
...
procedure TForm6.Timer_last_visitTimer(Sender: TObject); //Последний визит.
begin
  try
...
if Form1.Visible=False then
  ...
  Application.Restore;
  Form1.Visible:=False;
  if TrayIcon1.BalloonTitle='Новое сообщение' then N16.Click;
    ...
    begin
      ...
      if rating>=price then
        begin
          ...
          ZQueryNew2.SQL.Add(sql_p);
          ZQueryNew2.ExecSQL;
          ShowMessage('Приз получен!');
          N26.Click;
          ListBox1.ItemIndex:=ListBox1.Items.Count-1;
        end
      else ShowMessage('Недостаточно рейтинга!');
    end;
end;
...
begin
  ...
  Form6.TrayIcon1.BalloonHint:='Кликните чтоб развернуть.';
  Form6.TrayIcon1.BalloonTitle:='Программа свернута в трей.';
  Form6.TrayIcon1.ShowBalloonHint;
...
begin
  ...
  ShowMessage('Настройки изменены!');
  N17.Click;

end;

procedure TForm6.Button1Click(Sender: TObject); //Закрыть форму.
begin
...
procedure TForm6.Button2Click(Sender: TObject);   //Перезайти.
begin
  Form6.Visible:=False;
  Form1.Visible:=True;
end;

procedure TForm6.Button3Click(Sender: TObject); //Отправка сообщения в чат.
begin
  ChatSend();
...
begin
  ...
  ZQuery_otchet.ExecSQL;
  ShowMessage('Задание отправленно на проверку. Вы можете редактировать отчет задания на вкладке "На проверке"');
  Form6.N17.Click;
end;
...
begin
  ...
  ShowMessage('Сообщение отмечено как прочитанное!');
  Form6.N17.Click;
  ZQuery_otchet.ExecSQL;
...
begin
  if DockTabSet_massages.TabIndex>=0 then
    begin
      ...
      Memo_massages.Clear;
      Memo_massages.Lines.Add('+++++Сообщение+++++');
...
procedure TForm6.DockTabSet_taskClick(Sender: TObject);    //  Выбор номера задания
begin
  if DockTabSet_task.TabIndex>=0 then
    begin
      ...
      Memo_task.Clear;
      Memo_task.Lines.Add('+++++Ваше задание+++++');
...
procedure TForm6.ListBox_N18Click(Sender: TObject); //Выбор сотрудника из списка.
var
sql_N18 : string;
begin
...
procedure TForm6.ListBox_N20Click(Sender: TObject);   //Выбор собеседника. Загрузка последних 20 сообщений.
